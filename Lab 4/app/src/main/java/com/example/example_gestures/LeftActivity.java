...
public class LeftActivity extends Activity implements
            ...
            if (Math.abs(diffX) > Math.abs(diffY)) {
                if (Math.abs(diffX) > SWIPE_THRESHOLD && Math.abs(velocityX) > SWIPE_VELOCITY_THRESHOLD) {
                    if (diffX > 0) {
                        ...
                        Intent myIntent = new Intent(LeftActivity.this, MainActivity.class);
                        startActivity(myIntent);
                    } else {
                        Toast.makeText(getApplicationContext(), "Swipe Left." , Toast.LENGTH_SHORT).show();
                        Intent myIntent = new Intent(LeftActivity.this, RightActivity.class);
                        startActivity(myIntent);
                    }
                    result = true;
                }
